...
class SaveOutput extends Output {
	 ...
	 */
	public SaveOutput( String matchRegex, String matchJavaUtilPattern, String showRegexMatch, String matchCase, String type, String saveName, int splitIndex, String splitBy ) {
	...
	boolean match( Object o ) {
		...
		if (result && (getType() == TestCondition.REQUIRED || getType() == TestCondition.SUCCESS)) {
			...
			if (!line.contains(_splitBy)) {
				...
				if (_splitIndex >= arraySize || _splitIndex < 0) {
					System.out.printf("String split failure: splitIndex out of bound %n" + "Size of splitArray: %s%n"
							+ "splitIndex: %s%n", arraySize, _splitBy);
					result = false;
				} else {
					TestSuite.putVariable(_saveName, splitArray[_splitIndex]);
				}
			}
		}
		return result;
